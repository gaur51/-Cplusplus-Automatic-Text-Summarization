...
{   FILE *l=fopen("Sim_file.txt","w");
    FILE *m=fopen("stopwords.txt","r");
    int x;
    char st[50];
    vector<string> SW;
    while(!feof(m))
    ...
    //for(int x=0;x<SW.size();x++)cout<<"\t"<<SW[x];
    while(TRUE)
   {  int ch = getc(f);
      if (ch == EOF) return;
      if (LETTER(ch))
      {  int i = 0;
         ...
         for(x=0;x<SW.size();x++)
         {
             const char* str= SW[x].c_str();
             if(strcmp(str,s)==0){break;}
         }

         if(x==SW.size() && strlen(s)>2){fputs(s,l);fputs(" ",l);}
      }
      if(ch=='.') {fputc(ch,l); fputs("\n",l);}
      //else putchar(ch);
   }
}
